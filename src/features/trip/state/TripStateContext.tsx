...
import type { Match, Player, Round, ScoreEntry, Team, TeamId, Trip, TripState } from "@/types";
...
type TripStateContextValue = TripState & {
  ...
  updateMatchPlayer: (matchId: string, side: "A" | "B", index: number, playerId: string) => void;
...
export function TripStateProvider({ children }: { children: React.ReactNode }) {
      ...
      upsertScore: (score) => {
        setState((current) => {
          const exists = current.scores.some(
            (item) => item.roundId === score.roundId && item.playerId === score.playerId
          );
