import { courseHandicap, getScore, netScore, playerNetToPar, resolveMatch } from "@/lib/scoring";
...
export function MatchDetailScreen({
  ...
  const renderPlayerRows = (playerIds: string[]) =>
    playerIds.map((playerId) => {
      ...
      const net = score ? netScore(player, round, score.grossScore, courses) : null;
      const plusMinus = score ? playerNetToPar(player, round, score.grossScore, courses) : null;
        ...
        <div key={player.id} className="rounded-xl bg-slate-50 p-3 text-sm">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500">Index {player.handicapIndex} · CH {courseHandicap(player, round, courses)}</p>
            </div>
  ...
  return (
    <div className="space-y-4">
      <button onClick={() => setActiveScreen("matchCenter")} className="text-sm font-bold text-fairway-900">
        ...
        <div className="p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">{round.title} · {course.name}</p>
              <p className="mt-1 text-sm text-slate-500">{formatRoundFormat(round.format)}</p>
            </div>
            <Pill tone={result.status === "final" ? "green" : "amber"}>{result.status}</Pill>
          </div>
