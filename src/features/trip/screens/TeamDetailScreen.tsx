...
export function TeamDetailScreen({
  ...
  const summary = buildTeamSummaries(teams, players, rounds, matches, scores, courses).find((item) => item.teamId === team.id);
  ...
  return (
    <div className="space-y-4">
      <button onClick={() => setActiveScreen("teams")} className="text-sm font-bold text-fairway-900">
        ← Back to Teams
      </button>
      ...
      <Card className="p-5">
        <div className="flex items-start justify-between">
          <div>
            <h1 className={team.id === "A" ? "text-3xl font-black text-red-800" : "text-3xl font-black text-blue-800"}>
              {team.name}
            </h1>
            <p className="mt-1 text-sm text-slate-500">{roster.length} players</p>
          </div>
          <Pill tone={team.id === "A" ? "red" : "blue"}>{summary?.points ?? 0} pts</Pill>
        </div>

        <div className="mt-5 grid grid-cols-2 gap-2 text-center">
          <div className="rounded-xl bg-slate-50 p-3">
            <p className="text-xs font-bold text-slate-500">Avg HCP</p>
            <p className="mt-1 text-xl font-black">{summary?.averageHandicap.toFixed(1)}</p>
          </div>
          <div className="rounded-xl bg-slate-50 p-3">
            <p className="text-xs font-bold text-slate-500">Match Wins</p>
            <p className="mt-1 text-xl font-black">{summary?.completedMatches}</p>
          </div>
        </div>
      </Card>
            ...
            >
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  <p className="text-xs font-bold text-slate-500">Index {player.handicapIndex}</p>
            ...
            return (
              <div key={match.id} className="rounded-xl bg-slate-50 p-3 text-sm">
                <div className="flex items-center justify-between">
                  ...
                  <Pill tone={result.winner === team.id ? "green" : result.winner === "T" ? "amber" : "default"}>
                    {result.winner === team.id ? "Won" : result.winner === "T" ? "Tied" : result.status}
                  </Pill>
                </div>
