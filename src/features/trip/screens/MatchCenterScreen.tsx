...
export function MatchCenterScreen({
  ...
  return (
    <div className="space-y-4">
      <SectionHeader title="Match Center" subtitle="Pairings, formats, status, and point values." />
          ...
          >
            <Card className="p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    {round ? formatRoundFormat(round.format) : ""} · {match.points} pts
                  </p>
                </div>
              ...
              <div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-center gap-3 text-center">
                <p className="text-sm font-bold text-red-800">{match.aPlayers.map(getPlayerName).join(" + ")}</p>
                <p className="text-xs font-bold text-slate-400">VS</p>
                <p className="text-sm font-bold text-blue-800">{match.bPlayers.map(getPlayerName).join(" + ")}</p>
              </div>

              <p className="mt-4 rounded-xl bg-slate-50 p-3 text-sm font-semibold text-slate-600">{result.label}</p>
            </Card>
          </button>
        );
      })}
