...
export function OverviewScreen({ setActiveScreen }: { setActiveScreen: (screen: Screen) => void }) {
  const { trip, players, rounds, matches, scores, courses } = useTripState();
  const progress = getTournamentProgress(trip.totalPoints, matches, players, rounds, scores, courses);
    ...
    <div className="space-y-6">
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 h-4 overflow-hidden rounded-full bg-slate-100">
          <div className="h-full rounded-full bg-fairway-900" style={{ width: `${progress.progressPercent}%` }} />
        </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2 text-center">
          ...
          <div className="rounded-xl bg-sand-50 p-3">
            ...
            <p className="mt-1 text-xl font-black">{progress.remainingPoints}</p>
          </div>
          <div className="rounded-xl bg-slate-50 p-3">
      ...
      <div className="grid grid-cols-3 gap-2">
        <Card className="p-3 text-center">
          ...
          <p className="mt-1 text-sm font-black">{awards.mvp?.player.name ?? "-"}</p>
        </Card>
        <Card className="p-3 text-center">
          <p className="text-xs font-bold text-slate-500">🎯 Clutch</p>
          <p className="mt-1 text-sm font-black">{awards.clutch?.player.name ?? "-"}</p>
        </Card>
        <Card className="p-3 text-center">
          <p className="text-xs font-bold text-slate-500">❄️ Coldest</p>
          <p className="mt-1 text-sm font-black">{awards.coldest?.player.name ?? "-"}</p>
        </Card>
      </div>

      <div className="grid grid-cols-2 gap-3">
        <button onClick={() => setActiveScreen("matchCenter")} className="rounded-2xl bg-fairway-900 p-4 text-left text-white shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-2xl">⚔️</p>
              <p className="mt-2 font-black">Match Center</p>
              <p className="mt-1 text-xs text-white/75">Pairings and results</p>
            </div>
            <ChevronRight className="h-5 w-5 shrink-0 text-white/75" />
          </div>
        </button>

        <button onClick={() => setActiveScreen("teams")} className="rounded-2xl bg-sand-100 p-4 text-left text-slate-900 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-2xl">🏆</p>
              <p className="mt-2 font-black">Teams</p>
              <p className="mt-1 text-xs text-slate-500">Rosters and points</p>
            </div>
